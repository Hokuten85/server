UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 14; -- victory_smite
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 15; -- shijin_spiral
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 31; -- rudras_storm
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 224; -- exenterator
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 225; -- chant_du_cygne
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 226; -- requiescat
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 59; -- torcleaver
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 60; -- resolution
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 76; -- cloudsplitter
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 77; -- ruinator
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 92; -- ukkos_fury
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 93; -- upheaval
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 108; -- quietus
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 109; -- entropy
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 124; -- camlanns_torment
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 125; -- stardiver
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 140; -- blade_hi
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 141; -- blade_shun
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 156; -- tachi_fudo
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 157; -- tachi_shoha
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 173; -- dagan
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 174; -- realmrazer
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 190; -- myrkr
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 191; -- shattersoul
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 202; -- jishnus_radiance
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 203; -- apex_arrow
UPDATE weapon_skills SET skilllevel = 310, unlock_id = 0 WHERE weaponskillid = 220; -- wildfire
UPDATE weapon_skills SET skilllevel = 300 WHERE weaponskillid = 221; -- last_stand

UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 1; -- Combo
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 5; -- raging_fists
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 32; -- fast_blade
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 41; -- swift_blade
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 53; -- crescent_moon
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 69; -- rampage
UPDATE weapon_skills SET tertiary_sc = 1 WHERE weaponskillid = 69; -- raging_rush
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 102; -- guillotine
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 116; -- penta_thrust
UPDATE weapon_skills SET secondary_sc = 1 WHERE weaponskillid = 129; -- blade_retsu

UPDATE weapon_skills SET jobs = 0x00000000000100000002000000000000000001000000 WHERE weaponskillid = 23; -- dancing_edge
UPDATE weapon_skills SET jobs = 0x02000000000002020002000000000002000000000002 WHERE weaponskillid = 40; -- vorpal_blade	

UPDATE weapon_skills SET jobs = 0x00000000000000000000020100000000000000000000 WHERE weaponskillid = 192; -- flaming_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000020100000000000000000000 WHERE weaponskillid = 193; -- piercing_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000020100000000000000000000 WHERE weaponskillid = 194; -- dulling_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000020100000000000000000000 WHERE weaponskillid = 196; -- sidewinder
UPDATE weapon_skills SET jobs = 0x00000000000000000000010100000000000000000000 WHERE weaponskillid = 197; -- blast_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000010100000000000000000000 WHERE weaponskillid = 198; -- arching_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000010100000000000000000000 WHERE weaponskillid = 199; -- empyreal_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000020100000000000000000000 WHERE weaponskillid = 201; -- refulgent_arrow
UPDATE weapon_skills SET jobs = 0x00000000000000000000020100000000000000000000 WHERE weaponskillid = 202; -- jishnus_radiance

UPDATE weapon_skills SET jobs = 0x02020202000002000002000000020200000001020200 WHERE weaponskillid = 176; -- heavy_swing
UPDATE weapon_skills SET jobs = 0x02020202000002000002000000020200000001020200 WHERE weaponskillid = 177; -- rock_crusher
UPDATE weapon_skills SET jobs = 0x02020200000002000000000000000000000001020200 WHERE weaponskillid = 178; -- earth_crusher
UPDATE weapon_skills SET jobs = 0x02020202000002000002000000020200000001020200 WHERE weaponskillid = 179; -- starburst
UPDATE weapon_skills SET jobs = 0x02020200000002000000000000000000000001020200 WHERE weaponskillid = 180; -- sunburst
UPDATE weapon_skills SET jobs = 0x01010101000001000001000000010100000001010100 WHERE weaponskillid = 181; -- shell_crusher
UPDATE weapon_skills SET jobs = 0x01010101000001000001000000010100000001010100 WHERE weaponskillid = 182; -- full_swing
UPDATE weapon_skills SET jobs = 0x01010101000001000001000000010100000001010100 WHERE weaponskillid = 183; -- spirit_taker
UPDATE weapon_skills SET jobs = 0x01010101000001000001000000010100000001010100, animation = 67 WHERE weaponskillid = 184; -- retribution
UPDATE weapon_skills SET jobs = 0x00000001000000000000000000000000000001000000 WHERE weaponskillid = 186; -- vidohunir
UPDATE weapon_skills SET jobs = 0x00000000000000000000000000000100000001000000 WHERE weaponskillid = 187; -- garland_of_bliss
UPDATE weapon_skills SET jobs = 0x00000000000000000000000000000000000001010000 WHERE weaponskillid = 188; -- omniscience
UPDATE weapon_skills SET jobs = 0x00010000000001000000000000000100000001000100 WHERE weaponskillid = 189; -- cataclysm
UPDATE weapon_skills SET jobs = 0x00000001000000000000000000000100000001010000 WHERE weaponskillid = 190; -- myrkr
UPDATE weapon_skills SET jobs = 0x01010101000001000001000000010100000001010100 WHERE weaponskillid = 191; -- shattersoul

UPDATE weapon_skills SET jobs = 0x00000100000001000000000000000000000000000100 WHERE weaponskillid = 168; -- hexa_strike
