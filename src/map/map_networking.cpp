...
int32 MapNetworking::parse(uint8* buff, size_t* buffsize, MapSession* PSession)
{
    ...
    if (ref<uint16>(buff, 2) != PSession->server_packet_id)
    {
        ...
        // Stock LSB resend path: the client's byte-2 is "the last server
        // packet id I received". Patch the ack byte and timestamp on our
        // last-sent datagram and resend it.
