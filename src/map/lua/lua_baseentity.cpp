...
int8 CLuaBaseEntity::getShieldSize()
{
    ...
    if (m_PBaseEntity->objtype != TYPE_PC && m_PBaseEntity->objtype != TYPE_PET && m_PBaseEntity->objtype != TYPE_TRUST)
    {
...
void CLuaBaseEntity::setRespawnTime(const uint32 seconds) const
{
    ...
    PMob->m_RespawnTime  = std::chrono::seconds(std::min(300u, seconds));
    PMob->m_AllowRespawn = true;
...
void CLuaBaseEntity::Register()
{
    ...
    SOL_REGISTER("isUsingStaff", CLuaBaseEntity::isUsingStaff);
