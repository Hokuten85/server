...
abilityObject.onUseAbility = function(player, target, ability)
    ...
    local duration = 180 + player:getMod(xi.mod.SAMBA_DURATION) + (player:getJobPointLevel(xi.jp.SAMBA_DURATION) * 2)
    ...
    player:addStatusEffect(xi.effect.HASTE_SAMBA, { power = 1000 + player:getMerit(xi.merit.HASTE_SAMBA_EFFECT), duration = duration, origin = player })
