...
xi.job_utils.dancer.useStepAbility = function(player, target, ability, action, stepEffect)
    ...
    local hitRate          = xi.combat.physicalHitRate.getPhysicalHitRate(player, target, 100 + player:getMod(xi.mod.STEP_ACCURACY), xi.attackAnimation.RIGHT_ATTACK, false)
...
xi.job_utils.dancer.useDesperateFlourishAbility = function(player, target, ability, action)
    ...
    if
        math.random() <= xi.weaponskills.getHitRate(player, target, 100 + player:getJobPointLevel(xi.jp.FLOURISH_I_EFFECT), xi.attackAnimation.LEFT_ATTACK) or
    ...
    then
        ...
        if
            ...
            resistRate > 0.10 and                                                                  -- Check actual resistance.
...
xi.job_utils.dancer.useViolentFlourishAbility = function(player, target, ability, action)
    ...
    then
        ...
        if
            ...
            resistRate > 0.10                                                                       -- Check actual resistance.
        then
